...
public:
    ...
    void update(float dt) {
        ...
        eIntegral = constrain(eIntegral, -50.0, 50.0); 
...
void setup() {
    ...
    leftWheel.setPIDGains(150.0, 40.0, 5.0);
    rightWheel.setPIDGains(150.0, 40.0, 5.0);
...
void loop() {
    ...
    if (Serial.available() > 0) {
        float cmdV = Serial.parseFloat();
        float cmdOmega = Serial.parseFloat();
        
        // Clear buffer line
        while(Serial.available() > 0) Serial.read();

        // Calculate and apply individual wheel speeds
        float targetVL, targetVR;
        robot.inverseKinematics(cmdV, cmdOmega, targetVL, targetVR);
        
        leftWheel.setTargetVelocity(targetVL);
        rightWheel.setTargetVelocity(targetVR);
    }

    // 2. Continuous control and state execution loop
    ...
    if (dt >= LOOP_TIME_SEC) {
        ...
        // Process physics loops
        leftWheel.update(dt);
        rightWheel.update(dt);
        robot.forwardKinematics(leftWheel.getVelocity(), rightWheel.getVelocity(), dt);

       
       // 3. Send Feedback to Raspberry Pi (X, Y, Theta, Real_V, Real_Omega)
        Serial.print(robot.getX(), 3);        Serial.print(",");
        Serial.print(robot.getY(), 3);        Serial.print(",");
        Serial.print(robot.getTheta(), 3);    Serial.print(",");
        Serial.print(leftWheel.getVelocity(), 3);  Serial.print(",");
        Serial.println(rightWheel.getVelocity(), 3); // Ends with newline '\n'
    }
}